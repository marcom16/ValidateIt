...
var validateName = function validateName(str) {
  ...
  if (str.match(/^[A-Za-z]([a-zA-Z]|-|")+$/g)) return true;
  return false;
};
 ...
 * Shame Zaller
 */
var validateFullName = function validateFullName(str) {

  if (str.match(/^[A-Z](('[A-Z][a-z]+)|([a-z]+)|(\.[ ][A-Z][a-z]+))([ ][A-Z][\.])?([ ][A-Z][a-z]+)?/g)) return true;
  return false;
};
...
var validateEmail = function validateEmail(str) {
  if (str.match(/^([A-Z|a-z|0-9](\.|_){0,1})+[A-Z|a-z|0-9]\@([A-Z|a-z|0-9])+((\.){0,1}[A-Z|a-z|0-9]){2}\.[a-z]{2,3}$/g)) return true;
  return false;
};
...
var validatePassword = function validatePassword(str) {
  if (str.match(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/g)) return true;
  return false;
};
 ...
 */
var validatePhone = function validatePhone(str) {
  if (str.match(/^\s*(?:\+?(\d{1,3}))?([-. (]*(\d{3})[-. )]*)?((\d{3})[-. ]*(\d{2,4})(?:[-.x ]*(\d+))?)\s*$/g)) return true;
  return false;
};
 ...
 *  58701
 */
var validateZipCode = function validateZipCode(str) {
  if (str.match(/^\d{5}(\-?\d{4})?$/g)) return true;
  return false;
};
...
var validateImage = function validateImage(data) {
  if (!(data == undefined)) {
    if (data.size <= 3000000) {
      if (data.type == "image/jpeg" || data.type == "image/jpg" || data.type == "image/gif" || data.type == "image/png") return true;
      return false;
    }
    return false;
  }
...
var validateDoc = function validateDoc(data) {
  if (!(data == undefined)) {
    if (data.size <= 3000000) {
      if (data.type == "application/pdf" || data.type == "application/msword" || data.type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document" || data.type == "application/vnd.ms-excel" || data.type == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") return true;
      return false;
    }
    return false;
  }
